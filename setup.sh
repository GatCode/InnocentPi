...
if ! grep -Fq "interface wlan0" $DHCPCD; then
	cat config/dhcpcd.conf | tee -a $DHCPCD
...
cat config/dnsmasq.conf | tee /etc/dnsmasq.conf
...
cat config/hostapd.conf | tee /etc/hostapd/hostapd.conf 
